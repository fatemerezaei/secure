...
%(only for the pdftex engine)%\RequirePackage[no-math]{fontspec}%(only for the luatex or the xetex engine)
...
\institute{University of Massachusetts Amherst \and Technion}
%
...
In this paper, we investigate the  resilience of Bitcoin to blocking by  powerful network 
entities such as ISPs and governments.
By characterizing Bitcoin's communication patterns, we design classifiers that can distinguish (and therefore block) Bitcoin traffic even if  tunneled through an encrypted channel like Tor, and even if  mixed with background traffic (e.g., due to open webbrowser tabs). 
We also evaluate the reliability of our bitcoin detectors in the presence of  traffic obfuscation tools like Tor pluggable transports, showing that such obfuscation mechanisms are not enough to ensure 
blocking-resistant  access to Bitcoin, therefore arguing for traffic obfuscation mechanisms
that are tailored to cryptocurrency applications.

\keywords{\bc, traffic analysis,  blocking-resistance.}
\end{abstract}
